...
class PreprocessedDataset(Dataset):
    ...
    def __init__(self, img_dataset, dtype='train', val_percent=None):
        ...
        if 'paths' in self._data[dtype]:
            ...
            self._data[dtype]['paths'] = [self._data[dtype]['paths'][i]
                                          for i in sorted_idx]
    ...
    def vectorize_data(self, img_dataset):
        for dtype in ('train', 'test'):
            ...
            features = np.concatenate(
                [np.expand_dims(t[0], 0) for t in self._data[dtype]])
            labels = np.concatenate([np.expand_dims(t[1], 0)
                                     for t in self._data[dtype]])
    ...
    def normalize_data(self, img_dataset):
        for dtype in ('train', 'test'):
            ...
            data = np.concatenate([np.expand_dims(t[0], 0)
                                   for t in self._data[dtype]])

            # Normalize into [0,1]
            mins = np.tile(np.expand_dims(
                np.amin(data, 0), 0), (data.shape[0], 1))
            maxes = np.tile(np.expand_dims(
                np.amax(data, 0), 0), (data.shape[0], 1))
    ...
    def delete_imgs(self, delete_list):
        ...
        indexes = [i for i in range(
            len(self)) if self._data[dtype]['paths'][i] in delete_list]

        self._data[dtype]['features'] = np.delete(
            self._data[dtype]['features'], indexes, 0)
        self._data[dtype]['labels'] = np.delete(
            self._data[dtype]['labels'], indexes)
        self._data[dtype]['paths'] = np.delete(
            self._data[dtype]['paths'], indexes)
