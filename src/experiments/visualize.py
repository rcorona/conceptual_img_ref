...
def plot_data(args, data):
    ...
    if args.fn == 'mean':
        ...
        sys.exit('{} is not a recognized plot group fn!'.format(
            args.plot_group_fn))
...
def bar_data(args, data):
    ...
    # Create legend.
    if args.legend_labels:
        plt.legend(handles=bars, labels=args.legend_labels.split(),
                   fontsize=args.legend_font_sz)
...
def confusion_matrix(args, data):
    ...
    attribute_scores = {
        t: attribute_scores[t] / score_sum for t in attribute_scores}
