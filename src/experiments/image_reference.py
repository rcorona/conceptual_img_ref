...
class ImgRefDataset(Dataset):
    ...
    def __init__(self, pops, n_samples, n_attrs,
        ...
        self._dtype = 'train'
        self._pops = pops
        self._pop_agents = torch.stack(
            [torch.from_numpy(p) for p in pops._cluster_attrs]).float()
        ...
        unique = np.tile(np.expand_dims(unique, 0),
                         (self._pop_agents.size(0), 1))
    ...
    def ref_game(self):
        ...
        n_imgs = self._batch_sz * \
            (self._n_samples + (self._eval_idx_len + 1) * self._n_targets) * 2

        # First compute for speaker.
        img_idx = np.random.choice(
            len(self._speaker_dataset), size=n_imgs, replace=True)
        imgs = np.reshape(self._speaker_dataset[img_idx], (2, n_imgs // 2, -1))

        speaker_diffs = torch.tensor(imgs[0, :, :] - imgs[1, :, :])
        speaker_diffs = speaker_diffs.view(
            self._batch_sz, -1, self._n_attrs).float()

        if self._speaker_dataset is self._listener_dataset:
            listener_diffs = speaker_diffs
        else:
            imgs = np.reshape(
                self._listener_dataset[img_idx], (2, n_imgs // 2, -1))

            listener_diffs = torch.tensor(imgs[0, :, :] - imgs[1, :, :])
            listener_diffs = listener_diffs.view(
                self._batch_sz, -1, self._n_attrs).float()
    ...
    for idx in range(start_idx, end_idx):
        ...
        if idx + 1 in eval_idxs:
            ...
            q_loss, guess_attrs, guess_r, eval_agent_embeddings, _ = play_games(model, dataset,
                                                                                ...
                                                                                True)
            eval_data.append(
                (q_loss, guess_attrs, guess_r, eval_agent_embeddings))
            eval_idx += dataset._n_targets
...
def train(model, dataset, n_updates, device, results_dir, curriculum,
    ...
    if results_dir:
        writer = csv.writer(
            open(os.path.join(results_dir, 'results.csv'), 'w'))
    ...
    for i in range(n_updates // len(dataset)):
        ...
        for i_batch, data in enumerate(data_loader):
            ...
            if train_on_eval:
                ...
                if model._policy_type == 'active':
                    ...
                    active_weights = torch.zeros(
                        (1, q_loss.size(-1)), device=q_loss.device)
                    for e_idx in range(len(eval_data)):
                        e_loss = eval_data[e_idx][0]
                        active_rewards[:, :eval_idxs[e_idx]
                                       ] += e_loss.mean(dim=1, keepdim=True)
                        active_weights[:, :eval_idxs[e_idx]] += 1
                    active_term = model.active_loss(
                        active_rewards / active_weights)
                ...
                if model._policy_type == 'active':
                    active_term = model.active_loss(
                        eval_q_loss.mean(dim=-1, keepdim=True))
            ...
            for e_idx, (eq_loss, _, eguess_r, _) in zip(eval_idxs, eval_data):
                ...
                stats['avg_reward_{}'.format(e_idx)].append(
                    eguess_r.mean().item())
        ...
        print('Update: {}; MSE: {}; AVG Reward: {}'.format(
            (i+1)*len(dataset), mse, avg_reward))
...
def test(model, dataset, args):
    ...
    agent_embeddings = {idx: {'embeddings': [], 'labels': []}
    ...
    with torch.no_grad():
        for i_batch, data in enumerate(data_loader):
            ...
            n_imgs = dataset._batch_sz * \
                (dataset._n_samples + (dataset._eval_idx_len + 1)
                 * dataset._n_targets) * 2
            ...
            if store_embeddings:
                for i in range(len(eval_data)):
                    ...
                    labels = cluster_labels.view(-1,
                                                 1).repeat(1, eval_data[i][2].size(1))
                    labels = labels.view(-1)
                    embeddings = eval_data[i][2].view(-1,
                                                      eval_data[i][2].size(-1))

                    agent_embeddings[eval_idxs[i]]['embeddings'].append(
                        embeddings.cpu())
                    agent_embeddings[eval_idxs[i]
    ...
    if store_embeddings:
        for idx in eval_idxs:
            agent_embeddings[idx]['embeddings'] = torch.cat(
                agent_embeddings[idx]['embeddings']).numpy()
            agent_embeddings[idx]['labels'] = torch.cat(
                agent_embeddings[idx]['labels']).numpy()
    ...
    if args.results_dir:
        ...
        if store_embeddings:
            embeddings_path = os.path.join(
                args.results_dir, 'agent_embeddings.pkl')
...
def parse_args(args=None, namespace=None):
    ...
    parser = argparse.ArgumentParser(
        description='Run image reference game experiments.')
...
if __name__ == '__main__':
    ...
    speaker_dataset = PreprocessedDataset(
        args.speaker_dataset, dtype=args.mode)

    if args.speaker_dataset == args.listener_dataset:
        listener_dataset = speaker_dataset
    else:
        listener_dataset = PreprocessedDataset(
            args.listener_dataset, dtype=args.mode)

        # Make sure all images match.
        speaker_paths = set(
            speaker_dataset._data[speaker_dataset._dtype]['paths'])
        listener_paths = set(
            listener_dataset._data[listener_dataset._dtype]['paths'])
        ...
        new_speaker_paths = list(
            speaker_dataset._data[speaker_dataset._dtype]['paths'])
        new_listener_paths = list(
            listener_dataset._data[listener_dataset._dtype]['paths'])
    ...
    if args.mode == 'test':
        clusters_path = os.path.join(
            str(args.results_dir), 'listener_clusters.pkl')
