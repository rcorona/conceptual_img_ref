...
class Speaker(nn.Module):
    ...
    def __init__(self, n_features, mid_dim, embed_agents, policy_type='epsilon_greedy',
        ...
        self._lstm = nn.LSTM(n_features, mid_dim, batch_first=True, num_layers=self._n_layers)
        ...
        self._h = nn.Parameter(torch.empty((self._n_layers, 1, mid_dim,), requires_grad=True))
        nn.init.uniform_(self._h, -0.1, 0.1)

        self._c = nn.Parameter(torch.empty((self._n_layers, 1, mid_dim,), requires_grad=True))
    ...
    def concat_feature(self, agent_embedding, img_features):
        ...
        if not self._embed_agents: 
            agent_embedding = torch.zeros(agent_embedding.shape).to(agent_embedding.device)
            
        state = torch.cat((agent_embedding, img_features.to(agent_embedding.device)), dim=1)
    ...
    def update_agent_embedding(self, one_hot, agent_cell):
        ...
        agent_embedding, agent_cell = self._lstm(one_hot.unsqueeze(1), agent_cell)
    ...
    def step(self, agent_embedding, agent_cell, features, rewards, eval_true):
        ...
        # First embed state. 
        state_feature = self._state(features.to(agent_embedding.device))#.detach()
        ...
        one_hot[np.arange(attrs.size(0)), attrs.view(-1)] = guess_reward.squeeze()

        agent_embedding, agent_cell = self.update_agent_embedding(one_hot.to(Q.device), agent_cell)
